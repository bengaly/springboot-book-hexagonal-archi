package com.bkkonate.springbootbookhexagonalarchi.infrastructure.adapters.output.customizedexception;

import com.bkkonate.springbootbookhexagonalarchi.domain.exception.BookNotFound;
import com.bkkonate.springbootbookhexagonalarchi.infrastructure.adapters.output.customizedexception.data.response.ExceptionResponse;
...
public class CustomizedExceptionAdapter extends ResponseEntityExceptionHandler {
    ...
    public final ResponseEntity<Object> handleUserNotFoundException(BookNotFound ex, WebRequest request) {
        ExceptionResponse exceptionResponse = new ExceptionResponse(LocalDateTime.now(), ex.getMessage(), Arrays.asList(request.getDescription(false)));
